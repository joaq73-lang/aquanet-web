...
function Index() {
  ...
  const stepBValid = Boolean(
    stepAValid &&
      registerForm.documentoTipo &&
      registerForm.documentoNumero.trim().length >= 6,
  );
        ...
        <div className="mx-auto grid max-w-6xl overflow-hidden rounded-[32px] border border-white/70 bg-white/80 shadow-[0_24px_80px_-24px_rgba(4,76,155,0.35)] backdrop-blur lg:grid-cols-[420px_minmax(0,1fr)]">
          <div className="relative flex flex-col justify-between overflow-hidden rounded-[32px] bg-[#033a7a] p-7 text-white sm:p-8 lg:p-10">
            <div>
              ...
              <div className="mt-10 space-y-4">
                ...
                <p className="max-w-md text-sm leading-7 text-white/80">
                  Accede a tu oficina virtual de SEDAPAL para pagos, reclamos y trámites en un solo lugar.
                </p>
              </div>
            </div>
            ...
            <div className="mt-12 rounded-[28px] border border-white/10 bg-white/10 p-6 shadow-[0_16px_60px_-36px_rgba(0,0,0,0.35)] backdrop-blur">
              ...
              <p className="mt-4 text-sm leading-6 text-white/75">
                Esta plataforma está optimizada para que gestiones tu servicio de agua de forma rápida y segura.
              </p>
            </div>
          </div>
          ...
          <div className="flex-1 p-4 sm:p-5 lg:p-6 xl:p-8 overflow-hidden max-h-[calc(100vh-4rem)] lg:max-h-[calc(100vh-5rem)]">
            <div className="h-full overflow-y-auto pr-1">
              <div className="mb-4">
                <h2 className="text-2xl font-semibold text-foreground sm:text-[2.2rem]">{authView === "login" ? "Iniciar sesión" : authView === "register" ? "Registrarse" : "Recuperar contraseña"}</h2>
                ...
                <p className="mt-3 rounded-2xl border border-border bg-background px-3 py-2 text-xs text-muted-foreground">
                  Usa <span className="font-medium text-foreground">prueba@aquanet.test</span> / <span className="font-medium text-foreground">Test1234!</span> para probar el acceso.
                </p>
              )}            </div>

            {authView === "login" && (
              <form onSubmit={handleLogin} className="space-y-5 rounded-3xl border border-border bg-white p-5 shadow-sm">
            ...
            {authView === "register" && (
              <form onSubmit={handleRegister} className="space-y-4 rounded-3xl border border-border bg-white p-4 shadow-sm">
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="space-y-1.5">
                    <label className="block text-xs font-semibold text-foreground" htmlFor="suministro">
                      N° de Suministro
                    </label>
                    <input
                      id="suministro"
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-sm font-medium text-foreground" htmlFor="referencia">
                      Referencia de cobro
                    </label>
                    <input
                      id="referencia"
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="space-y-1.5">
                    <label className="block text-xs font-semibold text-foreground" htmlFor="documento-tipo">
                      Tipo de documento
                    </label>
                    <select
                      id="documento-tipo"
                      value={registerForm.documentoTipo}
                      onChange={(event) => updateRegisterField("documentoTipo", event.target.value)}
                      disabled={!stepAValid}
                      className="w-full border-b border-border bg-transparent px-0 py-3 text-sm text-foreground outline-none transition focus:border-primary disabled:cursor-not-allowed disabled:opacity-60"
                    >
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-xs font-semibold text-foreground" htmlFor="documento-numero">
                      N° de documento
                    </label>
                    <input
                      id="documento-numero"
                      value={registerForm.documentoNumero}
                      onChange={(event) => updateRegisterField("documentoNumero", event.target.value)}
                      disabled={!stepAValid}
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="space-y-1.5">
                    <label className="block text-xs font-semibold text-foreground" htmlFor="nombres">
                      Nombres
                    </label>
                    <input
                      id="nombres"
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-xs font-semibold text-foreground" htmlFor="apellido-paterno">
                      Apellido Paterno
                    </label>
                    <input
                      id="apellido-paterno"
                      value={registerForm.apellidoPaterno}
                      onChange={(event) => updateRegisterField("apellidoPaterno", event.target.value)}
                      disabled={!stepBValid}
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="space-y-1.5">
                    <label className="block text-xs font-semibold text-foreground" htmlFor="apellido-materno">
                      Apellido Materno
                    </label>
                    <input
                      id="apellido-materno"
                      value={registerForm.apellidoMaterno}
                      onChange={(event) => updateRegisterField("apellidoMaterno", event.target.value)}
                      disabled={!stepBValid}
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-xs font-semibold text-foreground" htmlFor="telefono">
                      Teléfono
                    </label>
                    <input
                      id="telefono"
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="space-y-1.5">
                    <label className="block text-xs font-semibold text-foreground" htmlFor="correo-registro">
                      Correo electrónico
                    </label>
                    <input
                      id="correo-registro"
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-xs font-semibold text-foreground" htmlFor="password-registro">
                      Contraseña
                    </label>
                    <input
                ...
                <div className="space-y-3">
                  <div className="space-y-1.5">
                    <label className="block text-xs font-semibold text-foreground" htmlFor="repetir-password">
                      Repetir contraseña
                    </label>
                    <input
                      id="repetir-password"
                      ...
                      onChange={(event) => updateRegisterField("repetirPassword", event.target.value)}
                      disabled={!stepCValid}
                  ...
                  <div className="rounded-2xl border border-border bg-background p-3 text-xs text-muted-foreground">
                    ...
                    <ul className="space-y-1.5">
                      ...
                      <li className={`flex items-center gap-2 ${passwordChecks.upper ? "text-emerald-600" : ""}`}>
                        <CheckCircle2 className="h-4 w-4" /> Al menos una mayúscula
                      </li>
                      <li className={`flex items-center gap-2 ${passwordChecks.lower ? "text-emerald-600" : ""}`}>
                        <CheckCircle2 className="h-4 w-4" /> Al menos una minúscula
                      </li>
                      <li className={`flex items-center gap-2 ${passwordChecks.number ? "text-emerald-600" : ""}`}>
                ...
                <div className="rounded-2xl border border-border bg-white p-3 shadow-sm">
                  ...
                  <p className="mt-2 text-xs text-muted-foreground">
                    Esta es una vista previa. La verificación real de reCAPTCHA se integrará después.
                  </p>
                </div>

                <p className="text-sm text-muted-foreground">
                  Al término del registro recibirás un código de verificación en tu correo, el cual
                  será solicitado la primera vez que ingreses al aplicativo.
                </p>
  ...
  return (
    ...
    >
      <div className="p-4 sm:p-8">
        <div className="mb-8">
          <h2 className="text-3xl sm:text-4xl font-bold text-primary-deep">¡Bienvenido a AQUANET!</h2>
